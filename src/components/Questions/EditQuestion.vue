<template>
  <div class="max-w-4xl mx-auto p-6">
    ...
    <div class="mb-8">
      ...
      <p class="text-gray-600">Tizimga yangi savol qo'shish uchun quyidagi questionani to'ldiring</p>
    </div>
    ...
    <!-- Preview Card -->
    <Card v-if="isquestionValid" class="mt-8 border border-blue-200 bg-blue-50/30">
      <template #header>
      ...
      <template #content>
        <div class="space-y-4">
          <div class="flex flex-wrap gap-2">
            <Badge :value="question.subject" severity="info" class="px-3 py-1" />
            <Badge
              :value="question.difficulty"
              :severity="getDifficultySeverity(question.difficulty)"
              class="px-3 py-1"
            />
            <Badge :value="`${question.points} ball`" severity="warning" class="px-3 py-1" />
          </div>
...
const questionId=router.currentRoute.value.params.slug
...
const getQuestionById=async ()=>{
  try{
    ...
    question.text=res.data.text
    question.answer=res.data.answer
    question.difficulty=res.data.difficulty
    question.points=res.data.points
    question.subject=res.data.subject
  }catch(err){
    console.log(err);
  }
}
...
const isquestionValid = computed(() => {
  return question.text.trim() && question.subject && question.difficulty && question.points > 0
})
...
const updateQuestion = async () => {
  ...
  try {
    const res = await axios.put(`/questions/${questionId}`, {
      text: question.text,
